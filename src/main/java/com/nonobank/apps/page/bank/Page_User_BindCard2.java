...
public class Page_User_BindCard2 extends BasePage {
	...
	public void input_validateCode(){
		...
		input_validateCode.input("0615");
		input_validateCode = objectFactory.getWebInput("validNo");
		String value = input_validateCode.getValue();
		Assert.assertEquals(value, "0615");
	}
	...
	public void isBindCardSuccess(){
		if(objectFactory.isElementExists("success", WebElementType.WebCommon)){
			...
			logger.error("绑卡失败");
			Assert.fail("binding card failed.");
		}
	}
}
